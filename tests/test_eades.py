...
class EadesTest(unittest.TestCase):
    ...
    def test_distance(self):
        node1 = GraphNode(canvas=tk.Canvas(), x=0, y=0, id=1, draw_ids=False)
        node2 = GraphNode(canvas=tk.Canvas(), x=0, y=200, id=2, draw_ids=False)
        self.assertEqual(distance(node1=node1, node2=node2), 200)

        node1 = GraphNode(canvas=tk.Canvas(), x=0, y=0, id=1, draw_ids=False)
        node2 = GraphNode(canvas=tk.Canvas(), x=200, y=0, id=2, draw_ids=False)
        self.assertEqual(distance(node1=node1, node2=node2), 200)

        node1 = GraphNode(canvas=tk.Canvas(), x=0, y=0, id=1, draw_ids=False)
        node2 = GraphNode(canvas=tk.Canvas(), x=100, y=100, id=2, draw_ids=False)
